...
class NN20NDRegModel(Model):
    def name(self):
        return 'nn20nd_reg' if not self.instance_name else f"nn20nd_reg_{self.instance_name}"
    ...
    def fit(self, training_matrix, training_labels, validation_matrix, validation_labels, dictionary):
        ...
        es = keras.callbacks.EarlyStopping(monitor='val_accuracy', min_delta=0, patience=50, verbose=0, mode='auto', restore_best_weights=True)
