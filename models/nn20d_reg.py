...
class NN20DRegModel(Model):
    def name(self):
        return 'nn20d_reg' if not self.instance_name else f"nn20d_reg_{self.instance_name}"
    ...
    def fit(self, training_matrix, training_labels, validation_matrix, validation_labels, dictionary):
        ...
        self.model = keras.models.Sequential([ # feed forward NN with a single hidden layer
                                            keras.layers.Dense(20, input_dim=num_features, activation='tanh', kernel_regularizer=regularizers.L2(0.01)),
