...
class LSTMDropBiDiModel(SequenceModel):
    ...
    def name(self):
        return f'lstm2_drop_bidi_128_d_2_75_l64_e{self.embedding_size}' if not self.instance_name else f"lstm2_drop_bidi_128_d_2_75_l64_e{self.embedding_size}_{self.instance_name}"
    ...
    def fit(self, training_matrix, training_labels, validation_matrix, validation_labels, dictionary):
        ...
        self.model = keras.models.Sequential([layers.Embedding(dictionary_size, self.embedding_size, input_length= np.shape(training_matrix)[1]),
                                            ...
                                            layers.Bidirectional(layers.LSTM(64, dropout=0.10)),
                                            layers.Dropout(0.10),
                                            # layers.Dropout(0.2),
