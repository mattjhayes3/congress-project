...
def run(save_dir, m, style, style_w_count, congresses, chamber):
    ...
    with open(f'splits/{chamber}{congresses}_{style_wo_gram}_train.txt', 'r') as f:
        all_training_files = f.read().split()
    with open(f'splits/{chamber}{congresses}_{style_wo_gram}_valid.txt', 'r') as f:
        all_validation_files = f.read().split()
    with open(f'splits/{chamber}{congresses}_{style_wo_gram}_test.txt', 'r') as f:
        all_test_files = f.read().split()

    ccs = f"{chamber}_{congresses}_{style}"
    ccss = f"{chamber}_{congresses}_{style_w_count}"
    ...
    if m.is_sequence():
        ...
        print(f"reading from matricies/{ccss}_matrix.txt.npz")
        feature_matrix = sparse.load_npz(
    ...
    print(f"files loaded {len(filenames_rows)} rows")
...
if __name__ == "__main__":
    ...
    # for m in [BoostModel()]: 
        ...
        # for style, style_w_count in [('small_0', 'small_0_10_50'), ('max_balanced_0', 'max_balanced_0_10_50')]: #
        ...
        # for style in ['3gram_max_balanced_0', '2gram_max_balanced_0']: # '097',
            for chamber in ['House']:
                ...
                np.random.seed(0)
                tf.random.set_seed(0)
                print(f"*** Running for {m.name()}, {congresses} ***")
                if not m.use_gpu():
                    with tf.device("/cpu:0"):
                        run(save_dir, m, style, style_w_count, congresses, chamber)
                else:
                    run(save_dir, m, style, style_w_count, congresses, chamber)
